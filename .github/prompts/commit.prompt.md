...
4. Write a commit message that summarizes the recent work to a temp file, then run `git commit -F <tempfile>` and delete the temp file afterward. Do NOT use `git commit -m` — multi-line messages with special characters break shell quoting.
